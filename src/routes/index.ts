...
export const getUser = async(id: number) => {
    return await axios.get('http://localhost:8080/users/' + id, {timeout: 1000})
}
...
export const deleteUser = async(id: number) => {
    return await axios.delete('http://localhost:8080/users/' + id, {timeout: 1000})
}
...
export const getProductsByPlace = async(place: string) => {
    return await axios.get('http://localhost:8080/product/place/' + place, {timeout: 1000})
}

export const getProduct = async(name: string) => {
    return await axios.get('http://localhost:8080/product/name/' + name, {timeout: 1000})
}
...
export const deleteProduct = async(name: string) => {
    return await axios.delete('http://localhost:8080/product/' + name, {timeout: 1000})
}
