...
export const getMarkup = async(cb_type: string, getItems: () => Promise<any>): Promise<InlineKeyboardButton[][]> => {
    return await (await getItems()).data.map((el: Product | Place) => {
        return [Markup.button.callback(el.name, el.name + cb_type)]
    })
}

export const getInlineKeyboard = (back: boolean, items: InlineKeyboardButton[][]) => {
    return Markup.inlineKeyboard([...items, 
