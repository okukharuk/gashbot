...
export const botAdmin = (bot: Telegraf<Context>) => {
    ...
    bot.on('callback_query', async(ctx) => {
        ...
        switch(type) {
            case 'placeOpened':
                const productMarkup = await getMarkup('', async() => await getProductsByPlace(label));
                console.log(productMarkup)
                ctx.reply(
                    'choose wisely',
                    getInlineKeyboard(true, productMarkup)
                )
        }
    ...
    bot.on('text', async(ctx) => {
        if(ctx.state.isAdmin)
            switch(textInputType[ctx.chat.id]){
                ...
                case 'addPlace':
                    textInputType[ctx.chat?.id || 0] = '';
                    await createPlace({name: ctx.message.text})
                        .then((res) => {
                            res
                ...
                case 'updateItemProperty':
                    try{
                        const data = {[propertyInputType[ctx.chat.id].kind]: +ctx.message.text ? Number(ctx.message.text) : ctx.message.text} 
                        const name = propertyInputType[ctx.chat.id].name;
                        console.log(data, name)
                        const result = await collections.product?.updateOne({ name: name }, { $set: data });
                    console.log(result)
                    result
...
const onAdmin = async(ctx: Context) => {
    ...
    ctx.reply(
        'Choose place:',
        getInlineKeyboard(false, placesMarkup)
    )
}
...
const onDeleteItem = async(ctx: Context) => {
    ...
    ctx.reply(
        `Delete Item:`, 
        getInlineKeyboard(true, deleteProductMarkup)
    )
}
...
const onUpdateItem = async(ctx: Context) => {
    ...
    ctx.reply(
        `Update Item:`, 
        getInlineKeyboard(true, updateProductMarkup)
    )
}
...
const onUpdatePropertyQuery = (ctx: Context, name: string, kind: string) => {
    ...
    propertyInputType[ctx.chat?.id || 0] = {name: name, kind: kind.toLowerCase()};
